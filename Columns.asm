...
# Assume argument registers are only valid at the start of a function call
#   If a function needs the argument, save it in a temporary
# We enforce temporaries should not be modified after a function call, 
#   so save values to the stack prior to modification within a function
...
draw_board_loop:
    beq  $s0, 78, draw_board_done # Loop 78 times (13 rows * 6 cols)

    # Load the value from game_board[i]
    sll  $t1, $s0, 2      # offset = i * 4
    add  $t2, $t0, $t1    # address = &game_board[i]
    lw   $s1, 0($t2)      # $s1 = game_board[i] (the color value)

    # Calculate 2D grid coordinates (row, col) from 1D index (i)
    # $s2 (row) = i / 6 (quotient)
    # $s3 (col) = i % 6 (remainder)
    li   $t3, 6
    divu $s0, $t3
    mflo $s2             # s2 = row
    mfhi $s3             # s3 = col
    
    # Set the drawing color based on the value stored in the game board.
    # This is a chain of checks. If the value matches, it jumps to set
    # the color and then draws the pixel.
    jal set_draw_colour_black
    beq     $s1, 0, draw_board_pixel
    jal set_draw_colour_blue
    beq     $s1, 1, draw_board_pixel
    jal set_draw_colour_green
    beq     $s1, 2, draw_board_pixel
    jal set_draw_colour_red
    beq     $s1, 3, draw_board_pixel
    jal set_draw_colour_orange
    beq     $s1, 4, draw_board_pixel
    jal set_draw_colour_purple
    beq     $s1, 5, draw_board_pixel
    jal set_draw_colour_yellow
    beq     $s1, 6, draw_board_pixel
    
draw_board_pixel:
    # Prepare arguments for draw_pixel:
    # $a0 = x (col), $a1 = y (row), $a2 = color (already set)
    move $a0, $s3
    move $a1, $s2
    
    # Call the function to draw the single pixel for this cell.
    jal draw_pixel
    
    # This jump is technically redundant because continue_board_loop is the next label,
    # but it makes the flow explicit.
    j    continue_board_loop
    
continue_board_loop:
    # Move to the next cell in the game board array.
    addi $s0, $s0, 1
    j    draw_board_loop
